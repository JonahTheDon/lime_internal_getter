class SOHEstimator:
    def __init__(
        ...
        if soh:
            self.soh = [soh for i in range(self.num_cells_series)]
        else:
            self.soh = [1.0 for i in range(self.num_cells_series)]
        self.__prev_soh = [None for i in range(self.num_cells_series)]
        self.__ah1 = 0.0
        self.__ah2 = 0.0
        self.__wait_flag = 0
        self.__check_key = 0
        ...
        self.__cumu_ah = 0.0
        self.__soc1 = [None for i in range(self.num_cells_series)]
        self.__soc2 = [None for i in range(self.num_cells_series)]

    def soh_estimator(
        self, current, time_diff, voltage, recal_flag, initial_state
    ):
        cellcurr = current / self.num_cells_parallel
        ...
        if self.__ah1 == 0.0 and self.__ah2 == 0.0 and (self.__wait_flag == 0):
            if self.__check_key == 0:
                ...
                for j in range(self.num_cells_series):
                    if self.__prev_soh[j]:
                        self.soh[j] = self.__prev_soh[j]
        if recal_flag:
            for j in range(self.num_cells_series):
                if float(voltage[j]) != 0:
